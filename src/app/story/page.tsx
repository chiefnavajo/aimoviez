...
function VideoPlayer({ season, onVote, isFullscreen, onToggleFullscreen }: VideoPlayerProps) {
  ...
  return (
    <div className="relative h-full bg-black overflow-hidden" onClick={handleTap}>
      ...
      {/* On desktop (md:), position arrows after the sidebar (w-56 = 224px) */}
      {completedSegments.length > 1 && (
        <div className="absolute left-2 md:left-60 top-[60%] -translate-y-1/2 z-20 flex flex-col items-center gap-2">
          {/* Up Arrow */}
          <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={(e) => {
